...
def plot_hydrograms(obs_dir, model_dir, wells_dir, scale, date_ini, format_date='%d/%m/%Y', lang='EN'):
    ...
    wells_dir: (str) Directory to well coordinates. Must contains columns "well", "x", "y", "layer".
...
def plot_fit(obs_dir, sim_dir_mod2obs, format_date='%d/%m/%Y', lang='EN'):
    ...
    obs_dir: (str) Directory to observation data. Format as John Doherty's Groundwater Utilities.
    sim_dir_mod2obs: (str) Directory to simulated data generated by John Doherty's "mod2obs" utility.
    format_date:(str) (Optional) Date format to build DataFrames. Default is '%d/%m/%Y'
    lang: (str) (Optional) Language. Can choose between EN and SP. Default is EN.
    """
    # Current working directory
...
def mapped_hydrograms(obs_dir, model_dir, EPSG, wells_dir, date_ini, format_date='%d/%m/%Y', lang='EN', grid_dir=None):
    ...
    obs_dir: (str) Directory to observation data. Format as John Doherty's Groundwater Utilities.
    sim_dir: (str) Directory to simulated data generated by John Doherty's "mod2smp" utility.
...
def generate_contours(hds_dir, gsf_dir, date_ini, layers, sps, EPSG, levels, dry_cells = -999.99):
    """
    Function to generate water level contours from MODFLOW-USG output file. 
    ...
    # usgarrdet instruction file
    with open(os.path.join(cwd, 'usgarrdet.in'), 'w') as g:
        g.writelines(hds_dir+'\n')
        g.writelines('h\n')
        g.writelines('g\n')
    ...
    for layer in layers:
        ...
        #Run usggridlay and exctract xyz data
